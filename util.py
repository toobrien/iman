...
path.append("/Users/taylor/trading/daily")

from    data.cat_df            import  get_futc


CONFIG      = loads(open("/Users/taylor/trading/daily/spreads/config.json").read())
...
def get_dbn_df(
    ...
    df  = pl.read_csv(
            join("~/trading/databento/csvs", f"{symbol}.c.0_ohlcv-1m.csv")
        ).with_columns(
            pl.col(
                "ts_event"
            ).cast(
